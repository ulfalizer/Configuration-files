...
# Like _err(), but adds the name of the calling function
_err_name() { _err "${FUNCNAME[1]}: $1"; }

# Helper function for printing a usage string
_usage() { _err "usage: ${FUNCNAME[1]} $1"; }
...
asm_() {
    ...
    if [[ ! -e $ofile ]]; then
        _err_name "no '$ofile' generated by g++. Aborting."
        return 1
    fi
